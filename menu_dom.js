...
function f33(p){
if(p&&p.indexOf(':/')==-1&&p.indexOf(':\\')==-1&&p.indexOf('index.html')!=0) return unescape(absPath)+p
else return p}
...
if(!itemWnd.status&&itemWnd.url.indexOf("<root>")!=-1)itemWnd.status=itemWnd.url.replace("<root>","index.html")
...
if(newUrl.length>0&&newUrl.substr(newUrl.length-1,1)=="index.html")newUrl=newUrl.substr(0,newUrl.length-1)
return newUrl}
function f60PU(pp){
var q=pp.q
